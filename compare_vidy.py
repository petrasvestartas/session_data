"""
Compare main_5 vidy_corner output against wood's reference (out.xml).
Usage: cd session_data && python compare_vidy.py
Wood reference: C:/brg/code_cpp/wood/cmake/src/wood/dataset/out.xml (42 polyline_groups)
Our output:     WoodF2F_vidy_corner.pb_coords.txt
"""
import xml.etree.ElementTree as ET
import os, math

COORDS_FILE = "WoodF2F_vidy_corner.pb_coords.txt"
WOOD_XML    = "ref_vidy.xml"
TOL         = 0.5  # mm

# ── Load our coordinate dump ──────────────────────────────────────────────────
our = {}  # {element_idx: [[pts_poly0], [pts_poly1], ...]}
...
# ── Load wood reference (out.xml) ─────────────────────────────────────────────
if not os.path.exists(WOOD_XML):
    print(f"ERROR: {WOOD_XML} not found — build+run wood cmake project first")
    exit(1)
...
if len(our) != len(wood_ref):
    print("ELEMENT COUNT MISMATCH — check that out.xml is the vidy_corner reference (42 groups)")
    print(f"  out.xml groups={len(wood_ref)} (expected 42 for vidy_corner)")
...
for ei in sorted(our.keys()):
    ...
    if len(ours) != len(ref):
        ...
        print(f"  our  counts: {[len(p) for p in ours]}")
        print(f"  wood counts: {[len(p) for p in ref]}")
        fail += 1
        continue
    el_ok = True
